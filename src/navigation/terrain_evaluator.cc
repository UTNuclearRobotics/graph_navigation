...
namespace motion_primitives
{
      ...
      const std::vector<std::shared_ptr<PathRolloutBase>> &paths)
  {
    ...
    if (cost_model_path_.length() != 0)
    {
      // Generate the scalar cost image using the neural network model.
      cost_image = GetScalarCostImage(latest_bev_image);
    }
    else
    {
      // If no model path is provided, use the first channel of the BEV image as the cost image.
      std::vector<cv::Mat> channels;
      cv::split(latest_bev_image, channels);
      channels[0].convertTo(cost_image, CV_32F);
    }

    // Convert the scalar cost image to an RGB image for visualization.
    latest_cost_image_ = GetRGBCostImage(cost_image);
    latest_cost_image_.copyTo(latest_vis_image_);

    // TODO: Skipped a bunch of code dealing with other factors: distance to goal,
    ...
    // Calculate terrain costs for each path.
    ...
    for (size_t i = 0; i < paths.size(); i++)
    {
      ...
      for (int j = 0; j <= CONFIG_rollout_density; j++)
      {
        ...
        // Check if the query point is within the bounds of the cost image.
        ...
        // TODO: Use the robot width and length from config.
    ...
    // Find the best path based on the calculated costs.
    ...
    for (size_t i = 0; i < paths.size(); ++i)
    {
      // Calculate the progress of the path towards the goal
      const float path_progress = local_target.norm() - endpoint_dist_to_goal[i];

      // Calculate the cost of the current path based on various weights and factors
      path_costs_[i] =
          CONFIG_dist_to_goal_weight * path_progress +
          CONFIG_clearance_weight * paths[i]->FPL() +
          CONFIG_clearance_weight * paths[i]->Clearance() +
          CONFIG_terrain_weight * terrain_costs[i]; // Factor in terrain cost

      // If the current path cost is less than the best path cost found so far
      if (path_costs_[i] < best_path_cost)
      {
        // Update the best path cost to the current path cost
        best_path_cost = path_costs_[i];

        // Update the best path to the current path
        best_path = paths[i];
      }
    }

    // Draw the path costs on the visualization image.
...
cv::Mat1f TerrainEvaluator::GetScalarCostImage(const cv::Mat3b &bev_image)
{
       ...
       row += CONFIG_patch_size_pixels)
  {
    ...
    {
      // Define the region of interest (ROI) for the current patch.
      const cv::Rect bev_patch_roi(col, row, CONFIG_patch_size_pixels, CONFIG_patch_size_pixels);

      // Clone the patch into its own Mat because the underlying data pointer of the submat
      // is of the original image. This ensures the patch data is independent of the original image.
      cv::Mat3b bev_patch = bev_image(bev_patch_roi).clone();

      // Skip areas that are not visible. These areas are filled with black pixels.
      ...
      // If the ratio of non-zero pixels is below the threshold, skip this patch.
      ...
      // Clone the tensor to take ownership of the underlying cv::Mat3b patch data that will go out of scope.
      bev_patch_tensor = bev_patch_tensor.detach().clone();

      // Convert the tensor from BGR to RGB format.
      bev_patch_tensor = bev_patch_tensor.flip(2);

      // Change the tensor shape from HWC (Height, Width, Channels) to CHW (Channels, Height, Width).
      bev_patch_tensor = bev_patch_tensor.permute({2, 0, 1});

      // Store the tensor and its corresponding ROI.
      bev_patch_tensors.push_back(bev_patch_tensor);
      bev_patch_rects.push_back(bev_patch_roi);
    }
  }

  // Stack all patch tensors into a single tensor and move it to the appropriate device (CPU or GPU).
